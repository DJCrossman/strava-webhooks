 ...
 * read from the user's ACTIVE app in their encrypted credential cookie (set on
 * the /signin screen), so no Strava environment variables are required. Env
 * values are used only as a fallback when no cookie is present.
 *
